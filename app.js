...
function freshState() {
  ...
  return { startDate: null, days, moodboard: { manifestation: '', items: [] } };
}
...
function completedCount() { return state.days.filter(isComplete).length; }
...
function renderSidebar() {
  ...
  document.getElementById('ringFill').style.strokeDashoffset = circ - (circ * done / 75);
  ...
  if (todayEl) {
    todayEl.textContent = 'today is ' + new Date().toLocaleDateString('en-US', {
...
function renderBoard()    { fillGrid('boardGrid',    enrichedDays()); }
function renderUpcoming() { fillGrid('upcomingGrid', enrichedDays().filter(d => d.status === 'ready'),    '🌿', 'All caught up'); }
function renderComplete() { fillGrid('completeGrid', enrichedDays().filter(d => d.status === 'complete'), '✨', 'No completed days yet :('); }

function fillGrid(id, days, emptyIcon, emptyMsg) {
  const grid = document.getElementById(id);
  if (!grid) return;
  if (!days.length && emptyMsg) {
    grid.innerHTML = `<div class="empty"><div class="empty-icon">${emptyIcon}</div><p>${emptyMsg}</p></div>`;
    return;
  }
...
function cardHTML(day) {
  ...
  const badge     = day.status === 'ready' ? 'ready' : day.status;
  return `
...
function toggleTask(dayId, taskId) {
  ...
  if (activeDayId === dayId) {
    const enriched = enrichedDays().find(d => d.id === dayId);
    renderModalTasks(dayObj);
    document.getElementById('mdComplete').disabled = !isComplete(dayObj);
  }
...
function renderData() {
  ...
  if (!tbody) return;
  tbody.innerHTML = enrichedDays().map(day => {
    const date    = dayDate(day.id);
    ...
    const taskCells = TASKS.map(t => {
      const on = !!day.tasks[t.id];
      return `<td><button type="button" class="chk ${on ? 'yes' : 'no'}" data-day="${day.id}" data-task="${t.id}">${on ? '✓' : ''}</button></td>`;
    }).join('');
    return `<tr>
      <td><strong>day ${String(day.id).padStart(2, '0')}</strong></td>
      <td style="color:var(--text2)">${dateStr}</td>
      ${taskCells}
      <td><span style="font-size:10px;padding:3px 8px;border-radius:20px;font-weight:500;letter-spacing:.04em;${statusStyle}">${badge}</span></td>
      <td><div class="tbl-bar"><div class="tbl-bar-fill" style="width:${pct}%"></div></div></td>
    </tr>`;
  }).join('');
}
...
function renderMoodboard() {
  ...
  if (stmt && !stmt.textContent.trim() && state.moodboard.manifestation) {
    stmt.textContent = state.moodboard.manifestation;
  }
  const grid = document.getElementById('mbGrid');
  if (!grid) return;
  const { items } = state.moodboard;
  grid.innerHTML = items.length
    ? items.map(mbItemHTML).join('')
    : '<div class="mb-empty">add images and affirmations to build your vision board</div>';
}
...
function openDayModal(id) {
  activeDayId = id;
  const day    = enrichedDays().find(d => d.id === id);
...
function renderModalTasks(dayObj) {
  ...
  container.querySelectorAll('.task-row').forEach(row => {
    row.addEventListener('click', () => {
      ...
      toggleTask(activeDayId, row.dataset.task);
    });
  });
}
    ...
    body.innerHTML = `
      ...
      <div class="form-group">
        ...
        <div class="color-swatches">
          ${SWATCHES.map((c, i) => `<div class="swatch ${i === 0 ? 'sel' : ''}" style="background:${c.bg}" data-i="${i}"></div>`).join('')}
        </div>
      </div>`;
...
function confirmAdd() {
  ...
  if (addType === 'image') {
    ...
    if (file) {
      ...
      const reader = new FileReader();
      reader.onload = () => {
        state.moodboard.items.push({ id: uid(), type: 'image', src: reader.result, caption: capVal || undefined });
        save(); closeAddModal(); renderMoodboard();
    ...
    const src   = uploadDataUrl || (urlEl && urlEl.value ? urlEl.value.trim() : '');
...
function toast(msg, durationMs) {
  ...
  toastTimer = setTimeout(() => el.classList.remove('show'), durationMs ?? 3000);
}
...
function init() {
  ...
  document.getElementById('btnCompleteToday').addEventListener('click', () => {
    const cur = enrichedDays().find(d => d.status === 'current');
  ...
  ['boardGrid', 'upcomingGrid', 'completeGrid'].forEach(gridId => {
    document.getElementById(gridId).addEventListener('click', e => {
      const card = e.target.closest('.day-card[data-id]');
      if (card) openDayModal(+card.dataset.id);
    });
  });

  document.getElementById('dataBody').addEventListener('click', e => {
    const btn = e.target.closest('.chk[data-day][data-task]');
    if (!btn) return;
    toggleTask(+btn.dataset.day, btn.dataset.task);
  });
  ...
  document.getElementById('mdComplete').addEventListener('click', () => {
    ...
    if (!isComplete(state.days[activeDayId - 1])) return;
    const id = activeDayId;
    closeDayModal(); render();
    toast(`✦ day ${id} complete — you're unstoppable ✦`);
  });

  document.getElementById('mbGrid').addEventListener('click', e => {
    const btn = e.target.closest('.mb-del[data-id]');
    if (!btn) return;
    state.moodboard.items = state.moodboard.items.filter(i => i.id !== btn.dataset.id);
    save(); renderMoodboard();
  });
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') { closeDayModal(); closeAddModal(); }
  });
